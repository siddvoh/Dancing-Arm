...
- UFactory xArm (7 joints) on the LAN, default IP `192.168.1.241`.
...
- Camera index falls back: if `--camera 0` doesn't open, the script scans indices 0..9 and uses the first working device, so it works regardless of whether the ZED Mini, a USB webcam, or anything else is plugged in.
...
MIT, open source. See [LICENSE](LICENSE). The beat tracker, pose bank, safety envelope, and recorder are all in this repo.
